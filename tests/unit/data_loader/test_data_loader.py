...
from tk_nn_classifier.data_loader import load_data_set, _select_data_reader, split_data_set
...
class DataLoaderTestCase(TestCase):
    ...
    def test_load_csv_data_set(self):
        csv_samples = list(load_data_set(self.config['csv_fields'], self.csv_file))
        self.assertEqual(len(csv_samples), 30)
        self.assertEqual(list(csv_samples[0].keys()),
                         ['full_text','advertiser_name', 'source_type'])
        csv_samples = list(load_data_set(self.config['csv_fields'], self.csv_file, train_mode=False))
        self.assertEqual(len(csv_samples), 30)
        self.assertEqual(list(csv_samples[0].keys()),
                         ['full_text','advertiser_name', 'source_type',
    ...
    def test_load_trxml_data_set(self):
        trxml_samples = list(load_data_set(self.config['trxml_fields'], self.trxml_dir))
        ...
        trxml_samples = list(load_data_set(self.config['trxml_fields'], self.trxml_dir, train_mode=False))
        self.assertEqual(len(trxml_samples), 10)
        self.assertEqual(list(trxml_samples[0].keys()),
                         ['sec_vacancy.0.sec_vacancy',
                          'derived_org_name.0.derived_org_name',
    ...
    def test_split_csv_data_set(self):
        ...
        train_data = list(load_data_set(self.config['csv_fields'], train_file))
        eval_data = list(load_data_set(self.config['csv_fields'], eval_file))
    ...
    def test_split_trxml_data_set(self):
        ...
        train_data = list(load_data_set(self.config['trxml_fields'], train_dir))
        eval_data = list(load_data_set(self.config['trxml_fields'], eval_dir))
